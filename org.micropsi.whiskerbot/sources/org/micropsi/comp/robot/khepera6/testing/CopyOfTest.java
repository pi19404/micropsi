...
import org.micropsi.comp.robot.khepera6.Khepera;
...
public class CopyOfTest {
	...
	private void initialize() {
		...
		*/
		khepera = new Khepera(command, answer, false);
	...
	public static void main(String[] args) {
		...
		int motorL = 0, motorR = 0, motorL_raw, motorR_raw, Ps2_raw, Ps5_raw, Ps4_raw, Ps3_raw;
		double Ps2, Ps5, h;
		int sum1 = 0;
    int sum2 = 0;
		...
		for (int i = 0;i<3000;i++){
		  ...
		  if (test.khepera.getProximitySensors() == 0){
		    sum1++;
		    System.out.println("getProxSens returned 0");
      }
          
		  Ps2_raw = Integer.valueOf(test.khepera.getPs2());
		  Ps5_raw = Integer.valueOf(test.khepera.getPs5());
      Ps3_raw = Integer.valueOf(test.khepera.getPs3());
      Ps4_raw = Integer.valueOf(test.khepera.getPs4());
      
			Ps2 = ((-1.0 * ((Integer.valueOf(Ps2_raw) - 80)/970.0)) + 0.5) * 2.0;
			Ps5 = ((-1.0 * ((Integer.valueOf(Ps5_raw) - 80)/970.0)) + 0.5) * 2.0;
			motorL_raw = (int)(Ps2 * 5);
			motorR_raw = (int)(Ps5 * 5);
			//test.wait(1000);
			
			h = Double.compare(motorL_raw, 0);
			if (h > 0)
				motorL = (int)Math.ceil(motorL_raw);
			else if (h < 0)
				motorL = (int)Math.floor(motorL_raw);
			else if (h == 0){
				motorL = 0;
				System.out.println("motorL = 0!");
			}
			h = Double.compare(motorR_raw, 0);
			if (h > 0)
				motorR = (int)Math.ceil(motorR_raw);
			else if (h < 0)
				motorR = (int)Math.floor(motorR_raw);
			else if (h == 0){
				motorR = 1;
        motorL = 1;
				System.out.println("motorR = 0!");
			}
      ...
      if(Ps3_raw > 800 || Ps4_raw > 800){
        motorL = -2;
        motorR = -2;
      }
      
      System.out.println("setSpeed_motor("+motorL+","+motorR+")");
      
      
			if (test.khepera.setSpeed_motor(String.valueOf(motorL), String.valueOf(motorR)) == 0){
			  sum2++;
			  System.out.println("setSpeed_motor returned 0");
			}
      System.out.println("sum1="+sum1+"\tsum2="+sum2);        
		}
		System.out.println("sum1="+sum1+"\tsum2="+sum2);
		System.out.println("Simulationsende!");
		test.khepera.shutdown();
	}
