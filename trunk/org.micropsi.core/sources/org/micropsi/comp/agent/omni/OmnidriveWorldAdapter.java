...
public class OmnidriveWorldAdapter implements AgentWorldAdapterIF, AgentControllerIF {
	...
	/*	if(agent.getAgentType() != AgentTypesIF.AGENT_MICROPSI_OMNI)
			throw new MicropsiException(500,Integer.toString(agent.getAgentType()));*/

		logger.debug("Initializing omnidriveWorldAdapter");
		...
		LightUrge tmp = new LightUrge("light_sensor_front");
		((LocalNetFacade)micropsi.getNet()).getSensorRegistry().registerSensorDataProvider(tmp);
		urges.put("LIGHT_SENSOR_FRONT",tmp);

		tmp = new LightUrge("light_sensor_right");
		((LocalNetFacade)micropsi.getNet()).getSensorRegistry().registerSensorDataProvider(tmp);
		urges.put("LIGHT_SENSOR_RIGHT",tmp);		

		tmp = new LightUrge("light_sensor_left");
		((LocalNetFacade)micropsi.getNet()).getSensorRegistry().registerSensorDataProvider(tmp);
		urges.put("LIGHT_SENSOR_LEFT",tmp);		
	}
